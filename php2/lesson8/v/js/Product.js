class Product {
    ...
    _render(){
      ...
      let $img_link = $('<a/>', {href: `?c=goods&act=open&id=${this.id}`});

      let $img = $('<img/>', {
          src: this.img+'/000/fff',
          alt: this.title,
        'data-id': this.id,
      });
      ...
      let $footer = $(`<p/>`);
      $footer.append(`<span class="item-price">${this.price}$</span>`);
      let $buy_btn = $('<a/>', {href: `?c=cart&act=add&id=${this.id}`,text: 'Купить'});
      $buy_btn.appendTo($footer);
